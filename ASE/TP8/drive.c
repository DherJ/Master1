...
void init(){
	unsigned int ncyl = 0, nsec = 0, sector_size = 0;
	unsigned int i;
	...
	if (HDA_MAXCYLINDER != ncyl){
		puts("Problème rencontré - Nombre de cylindres invalide à l'initialisation");
		exit(INVALID_GEOMETRY);
	}
	
	if (HDA_MAXSECTOR != nsec){
		puts("Problème rencontré - Nombre de secteurs invalide à l'initialisation");
		exit(INVALID_GEOMETRY);
	}
	
	if (HDA_SECTORSIZE != sector_size){
		puts("Problème rencontré - Taille des secteurs invalide à l'initialisation");
		exit(INVALID_GEOMETRY);
	}
