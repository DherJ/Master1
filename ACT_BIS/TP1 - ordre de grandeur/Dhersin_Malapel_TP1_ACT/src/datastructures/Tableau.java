...
public class Tableau implements DataStructure {
	...
	public void mystere() {
		...
		for ( int i = 0 ; i < this.tab.length; i++){ // n itérations

			indexMin = i;
			for(int j=i+1;j<this.tab.length;j++){   // de l'ordre de n : pire des cas on a n-1 itérations
			...
			for (int k = indexMin; k < (indexMin  + this.tab.length)/2 ; k++){ // de l'ordre de log(n)
				tmp = this.tab[k];
				this.tab[k] = this.tab[this.tab.length + indexMin - k - 1]; // permute tous les termes à partir de l'indice du min 
				this.tab[this.tab.length + indexMin - k - 1] = tmp;         // avec les derniers éléments du tableau en partant de la fin
			}

			for (int p = i; p < (i  + this.tab.length)/2 ; p++){  // de l'ordre de n
				tmp = this.tab[p];
				this.tab[p] = this.tab[this.tab.length + i - p - 1];  // même chose que pour la précédente boucle mais en partant de l'indice i
