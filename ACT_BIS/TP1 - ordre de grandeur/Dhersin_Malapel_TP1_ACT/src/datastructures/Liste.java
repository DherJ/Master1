...
public class Liste extends ArrayList<Integer> implements DataStructure {
	...
	public void mystere() {
		...
		for (int i=0 ; i<this.size ; i++){
			...
			min = (Integer)Collections.min(this.subList(i,this.size)); // de l'ordre de n
			indexMin = this.indexOf(min);

			Collections.reverse(this.subList(indexMin,this.size));  // de l'odre de log(n)
			Collections.reverse(this.subList(i,this.size));         // de l'odre de log(n)
